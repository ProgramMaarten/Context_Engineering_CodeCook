...
class TodoApp {
  constructor() {
    ...
    this.todos = loaded.map(t =>
      Object.assign({ deadline: null }, t)
    );
  ...
  normalizeDatetimeLocal(raw) {
    ...
    // keep groups explicit and readable
    const datePart = m[1];
    const hh = m[2];
    const mm = m[3];

    // if time is 00:00 normalize to 23:59
    if (Number(hh) === 0 && Number(mm) === 0) return `${datePart}T23:59`;
  ...
  _renderTodoItem(todo) {
    ...
    const formattedDeadline = this._formatDeadline(todo.deadline);
    li.innerHTML = [
      `<span class="drag-handle">☰</span>`,
      `<input class="todo-checkbox" type="checkbox" ${todo.completed ? 'checked' : ''}>`,
      `<span class="todo-text">${this._esc(todo.text)}</span>`,
      `<span class="todo-deadline">${this._esc(formattedDeadline)}</span>`,
    ...
    li.addEventListener('dragstart', (e) => {
      try {
        ...
        // ignore - fallback to this.draggedId
      }
    ...
    li.addEventListener('drop', (e) => {
      ...
      const dropBelowMiddle = !(e.clientY < rect.top + rect.height / 2);
      if (dropBelowMiddle) to++;
  ...
  _updateCount() {
    ...
    const noun = total === 1 ? 'taak' : 'taken';
    this.todoCount.textContent = `${remaining} van ${total} ${noun} resterend`;
  }

  _clearDragOver() {
    const els = this.todoList.querySelectorAll('.drag-over');
  ...
  _add() {
    ...
    let deadlineIso;
    if (raw) {
      deadlineIso = new Date(this.normalizeDatetimeLocal(raw)).toISOString();
    } else {
      const t = new Date();
  ...
  _reattach(el) {
    ...
    el.querySelector('.todo-checkbox').addEventListener('change', () => this._toggleComplete(id));
    el.querySelector('.edit-btn').addEventListener('click', () => this._enterEdit(id));
    el.querySelector('.delete-btn').addEventListener('click', () => this._delete(id));
  }
